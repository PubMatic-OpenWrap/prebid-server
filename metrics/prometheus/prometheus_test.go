...
func TestRecordTLSHandshakeTime(t *testing.T) {
	...
	for i, test := range testCases {
		...
		histogram, found := getHistogramFromHistogramVec(pm.tlsHandhakeTimer, adapterLabel, string(test.in.adapterName))
		assert.True(t, found)
...
func TestRecordPodImpGenTime(t *testing.T) {
	...
	testAlgorithmMetrics(t, impressions, func(m *Metrics) dto.Histogram {
		...
		hist, _ := getHistogramFromHistogramVec(m.podImpGenTimer, podNoOfImpressions, strconv.Itoa(impressions))
		return hist
	})
}
...
func TestRecordPodCombGenTime(t *testing.T) {
	...
	testAlgorithmMetrics(t, combinations, func(m *Metrics) dto.Histogram {
		...
		hist, _ := getHistogramFromHistogramVec(m.podCombGenTimer, podTotalCombinations, strconv.Itoa(combinations))
		return hist
	})
}
...
func TestRecordPodCompetitiveExclusionTime(t *testing.T) {
	...
	testAlgorithmMetrics(t, totalBids, func(m *Metrics) dto.Histogram {
		...
		hist, _ := getHistogramFromHistogramVec(m.podCompExclTimer, podNoOfResponseBids, strconv.Itoa(totalBids))
		return hist
	})
}
	...
	for _, test := range testCases {
		t.Run(test.description, func(t *testing.T) {
			...
			for adapterName, adDurations := range test.bidderAdDurations {
				...
				result, _ := getHistogramFromHistogramVec(m.adapterVideoBidDuration, adapterLabel, adapterName)
