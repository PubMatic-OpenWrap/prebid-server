...
func (me *Metrics) RecordTLSHandshakeTime(adapterName openrtb_ext.BidderName, tlsHandshakeTime time.Duration) {
	...
	if !ok {
		logger.Errorf("Trying to log adapter TLS Handshake metrics for %s: adapter not found", string(adapterName))
		return
	}
...
func (me *Metrics) RecordAdapterThrottled(adapterName openrtb_ext.BidderName) {
	...
	am, ok := me.AdapterMetrics[strings.ToLower(adapterStr)]
	if !ok {
		logger.Errorf("Trying to log adapter throttled metric for %s: adapter not found", adapterStr)
		return
	}
