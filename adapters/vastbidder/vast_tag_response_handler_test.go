...
func TestGetAdvertisers(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, len(tc.expected), len(result), fmt.Sprintf("Expected slice length - %+v \nResult slice length - %+v", len(tc.expected), len(result)))

			for i, expected := range tc.expected {
				assert.Equal(t, result[i], expected, fmt.Sprintf("Element mismatch at position %d.\nExpected - %s\nActual - %s", i, expected, result[i]))
			}
		})
	}
}
