...
type reqBodyExt struct {
	VisibleMeasuresBidderExt reqBodyExtBidder `json:"bidder"`
}
...
func (a *adapter) MakeRequests(request *openrtb2.BidRequest, reqInfo *adapters.ExtraRequestInfo) ([]*adapters.RequestData, []error) {
	...
	for _, imp := range request.Imp {
		...
		temp := reqBodyExt{VisibleMeasuresBidderExt: reqBodyExtBidder{}}

		if visiblemeasuresExt.PlacementID != "" {
			temp.VisibleMeasuresBidderExt.PlacementID = visiblemeasuresExt.PlacementID
			temp.VisibleMeasuresBidderExt.Type = "publisher"
		} else if visiblemeasuresExt.EndpointID != "" {
			temp.VisibleMeasuresBidderExt.EndpointID = visiblemeasuresExt.EndpointID
			temp.VisibleMeasuresBidderExt.Type = "network"
		}
