...
type AccountFetcher interface {
	...
	FetchAccount(ctx context.Context, accountDefaultsJSON json.RawMessage, accountID string) (json.RawMessage, []error)
}
...
func (f *fetcherWithCache) FetchAccount(ctx context.Context, accountDefaultsJSON json.RawMessage, accountID string) (account json.RawMessage, errs []error) {
	...
	account, errs = f.fetcher.FetchAccount(ctx, accountDefaultsJSON, accountID)
	if len(errs) == 0 {
