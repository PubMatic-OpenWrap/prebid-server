...
func (db *mySqlDB) GetActivePartnerConfigurations(pubID, profileID int, displayVersion int) (map[int]map[string]string, error) {
	versionID, displayVersionID, platform, profileType, err := db.getVersionIdAndProfileDeatails(profileID, displayVersion, pubID)
	if err != nil {
		return nil, err
	}
...
func (db *mySqlDB) getVersionIdAndProfileDeatails(profileID, displayVersion, pubID int) (int, int, string, int, error) {
	var row *sql.Row
	if displayVersion == 0 {
