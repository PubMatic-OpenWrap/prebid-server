...
import (
	...
	"github.com/prebid/prebid-server/v4/modules/pubmatic/openwrap/database"
...
func TestCache_GetPerformanceDSPs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				mockDatabase.EXPECT().GetPerformanceDSPs().Return(map[int]struct{}{
					1: {},
					2: {},
				}, nil)
			},
		...
		},
		{
			...
			want: nil,
			setup: func() {
				mockDatabase.EXPECT().GetPerformanceDSPs().Return(nil, errors.New("QUERY FAILD"))
				mockEngine.EXPECT().RecordDBQueryFailure(models.PerformanceDSPsQuery, "", "").Return()
			},
			fields: fields{
				cache: gocache.New(100, 100),
				db:    mockDatabase,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.GetPerformanceDSPs()
			if (err != nil) != tt.wantErr {
				t.Errorf("cache.GetPerformanceDSPs() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
