...
type MetricsEngine interface {
	// RecordOpenWrapServerPanicStatRecord
	...
	RecordPublisherInvalidProfileRequests(publisher, profileID string)
	...
	RecordAMPPublisherRequests(publisher string)
	RecordAMPCacheErrorRequests(publisher, profileID string)
	RecordPublisherInvalidProfileAMPRequests(publisher, profileID string)
	RecordVideoBadRequests()
	RecordVideoPublisherRequests(publisher string)
	RecordVideoCacheErrorRequests(publisher, profileID string)
	RecordPublisherInvalidProfileVideoRequests(publisher, profileID string)
	Record25BadRequests()
	RecordAMPBadRequests()
	Record25PublisherRequests(publisher, platform string) // ???
	...
	RecordVideoImpDisabledViaConfigStats(publisher, profileID string)
	RecordBannerImpDisabledViaConfigStats(publisher, profileID string)
	...
	RecordCTVBadRequests(endpoint string, errorCode int)
	RecordCTVPublisherRequests(endpoint string, publisher string, platform string)
	...
	RecordementAdPodImpressionYield(maxDuration int, minDuration int, publisher string)
	...
	RecordidResponseByDealCountInPBS(publisher, profile, aliasBidder, dealId string)
	RecordidResponseByDealCountInHB(publisher, profile, aliasBidder, dealId string)
