...
func Test_isAllPartnerThrottle(t *testing.T) {
	type args struct {
		partnerConfigMap   map[int]map[string]string
	...
	tests := []struct {
		...
		{
			name: "empty_partnerConfigMap",
			args: args{
				partnerConfigMap:   map[int]map[string]string{},
				adapterThrottleMap: map[string]struct{}{"bidderA": {}},
			},
			want: false,
		},
		{
			name: "bidderCode_missing_or_empty",
			args: args{
				partnerConfigMap: map[int]map[string]string{
					1: {},
					2: {models.BidderCode: ""},
		...
		{
			name: "nil_adapterThrottleMap",
			args: args{
				partnerConfigMap: map[int]map[string]string{
					1: {models.BidderCode: "bidderA"},
				},
				adapterThrottleMap: nil,
			},
			want: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, isAllPartnerThrottle(tt.args.partnerConfigMap, tt.args.adapterThrottleMap))
		})
	}
}
