...
func TestOpenWrap_applyVideoAdUnitConfig(t *testing.T) {
	...
	featurereloader.ResetInitReloaderTest()
	featurereloader.Init(mockCache, 0)
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() func() {
				return featurereloader.SetAndResetAmpMultiformatWithMockCache(mockCache, map[int]struct{}{5890: {}})
			},
		},
		{
...
func TestOpenWrapHandleBeforeValidationHook(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fmt.Println("got:", got.DebugMessages)
