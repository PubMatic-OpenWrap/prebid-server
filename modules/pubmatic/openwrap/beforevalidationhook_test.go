...
func TestOpenWrap_handleBeforeValidationHook(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "draft profile (mapping not uploaded)",
			args: args{
				ctx: context.Background(),
				moduleCtx: hookstage.ModuleInvocationContext{
					ModuleContext: hookstage.ModuleContext{
			...
			setup: func() {
				...
				mockEngine.EXPECT().RecordPublisherProfileRequests("5890", "1234")
				mockEngine.EXPECT().RecordPublisherRequests(rctx.Endpoint, "5890", rctx.Platform)
				mockEngine.EXPECT().RecordPartnerConfigErrors("5890", "1234", "appnexus", models.PartnerErrSlotNotMapped)
				mockEngine.EXPECT().RecordBadRequests("v25", 6)
				mockEngine.EXPECT().RecordNobidErrPrebidServerRequests("5890", 613)
			},
			want: hookstage.HookResult[hookstage.BeforeValidationRequestPayload]{
				Reject:  true,
				NbrCode: 613,
				Errors:  []string{"mappings not found for imp:123 partner: appnexus", "slot not mapped"},
			},
			wantErr: false,
		},
	}
