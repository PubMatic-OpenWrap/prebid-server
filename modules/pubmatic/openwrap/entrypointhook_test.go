...
func TestOpenWrap_handleEntrypointHook(t *testing.T) {
	...
	clearInstance := wakanda.TestInstance()
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				setup: func(mme *mock_metrics.MockMetricsEngine) {
					mockFeature.EXPECT().GetApplovinMultiFloors(5890, "12929").Return(models.ApplovinAdUnitFloors{
		...
		},
		{
			...
			args: args{
				...
				setup: func(mme *mock_metrics.MockMetricsEngine) {
					mockFeature.EXPECT().GetApplovinMultiFloors(111, "222").Return(models.ApplovinAdUnitFloors{
						"adunit_name": {5.1, 2.1, 4, 4.3},
					})
					mockFeature.EXPECT().IsApplovinMultiFloorsEnabled(111, "222").Return(true)
				},
			},
			want: hookstage.HookResult[hookstage.EntrypointPayload]{
				ModuleContext: hookstage.ModuleContext{
