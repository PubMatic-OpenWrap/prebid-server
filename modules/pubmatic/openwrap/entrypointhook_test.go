...
func TestOpenWrap_handleEntrypointHook(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid /openrtb2/auction request with source=inapp",
			fields: fields{
				cfg: config.Config{
			...
			args: args{
				...
				payload: hookstage.EntrypointPayload{
					Request: func() *http.Request {
						r, err := http.NewRequest("POST", "http://localhost/openrtb2/auction?debug=1&source=inapp", nil)
						if err != nil {
							panic(err)
						}
		...
		{
			name: "valid /openrtb2/auction?source=inapp request with wiid set and no cookies",
			fields: fields{
				cfg: config.Config{
					Tracker: config.Tracker{
						Endpoint:                  "t.pubmatic.com",
		...
		{
			name: "/openrtb2/auction?source=inapp request without profileid",
			fields: fields{
				cfg:   config.Config{},
				cache: nil,
			},
			args: args{
				...
				payload: hookstage.EntrypointPayload{
					Request: func() *http.Request {
						r, err := http.NewRequest("POST", "http://localhost/openrtb2/auction?&source=inapp", nil)
						if err != nil {
							panic(err)
						}
						return r
		...
		{
			name: "/openrtb2/auction without source=pbjs. new hybrid endpoint should not execute module",
			fields: fields{
				cfg:   config.Config{},
				cache: nil,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want.ModuleContext != nil {
				...
				if gotRctx.Endpoint == models.EndpointHybrid {
