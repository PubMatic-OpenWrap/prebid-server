...
func TestOpenWrap_handleEntrypointHook(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid applovin max /openrtb2/auction?source=owsdk&agent=max request directly on port 8000",
			fields: fields{
				cfg: config.Config{
					Tracker: config.Tracker{
						Endpoint:                  "t.pubmatic.com",
			...
			args: args{
				...
				payload: hookstage.EntrypointPayload{
					...
					Body: []byte(`{"user":{"data":[{"id":"1","name":"Publisher Passed","segment":[{"signal":"{BIDDING_SIGNAL}"}]}],"ext":{"gdpr":0}},"ext":{"wrapper":{"profileid":5890,"versionid":1}}}`),
				},
				setup: func(mme *mock_metrics.MockMetricsEngine) {},
			},
			want: hookstage.HookResult[hookstage.EntrypointPayload]{
