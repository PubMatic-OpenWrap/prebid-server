...
func TestOpenWrap_handleEntrypointHook(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "only device in signal for applovin max /openrtb2/auction?source=owsdk&agent=max request",
			fields: fields{
				cfg: config.Config{
					Tracker: config.Tracker{
						Endpoint:                  "t.pubmatic.com",
			...
			args: args{
				...
				payload: hookstage.EntrypointPayload{
					...
					Body: []byte(`{"user":{"data":[{"id":"1","name":"Publisher Passed","segment":[{"signal":"{\"device\":{\"devicetype\":4,\"w\":393,\"h\":852}}"}]}],"ext":{"gdpr":0}},"ext":{"prebid":{"bidderparams":{"pubmatic":{"wrapper":{"profileid":123456,"versionid":1}}}}}}`),
				},
				setup: func(mme *mock_metrics.MockMetricsEngine) {},
			},
			want: hookstage.HookResult[hookstage.EntrypointPayload]{
			...
			doMutate: true,
			wantErr:  nil,
			wantBody: []byte(`{"id":"","imp":null,"device":{},"user":{"data":[{"id":"1","name":"Publisher Passed","segment":[{}]}],"ext":{"gdpr":0}},"regs":{},"ext":{"prebid":{"bidderparams":{"pubmatic":{"wrapper":{"profileid":123456,"versionid":1,"clientconfig":0}}}}}}`),
		},
		{
