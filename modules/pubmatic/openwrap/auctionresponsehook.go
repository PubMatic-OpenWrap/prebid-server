...
func applyBidExpAndBidExtFromCtx(rctx models.RequestCtx, bidResponse *openrtb2.BidResponse) {
	for i, seatBid := range bidResponse.SeatBid {
		for j, bid := range seatBid.Bid {
			...
			if rctx.Endpoint == models.EndpointWebS2S {
				// Preserve bidder-specific ext generated by PBS.
				if len(bidResponse.SeatBid[i].Bid[j].Ext) != 0 {
					bidResponse.SeatBid[i].Bid[j].Ext = jsonparser.Delete(
						bidResponse.SeatBid[i].Bid[j].Ext,
						models.BidExtBidExpEnf,
					)
				}
				continue
			}

			// Existing behavior for AppLovin/2.5 and other endpoints.
