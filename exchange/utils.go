...
func updateContentObjectForBidder(allBidderRequests []BidderRequest, requestExt *openrtb_ext.ExtRequest) {
	...
	if requestExt != nil && requestExt.Prebid.Transparency != nil {
		...
		var (
			rule openrtb_ext.BidderContentTransparency
			ok   bool
		)

		rules, defaultRule := getTransparencyRules(requestExt.Prebid.Transparency)
		...
		for i, bidderRequest := range allBidderRequests {
			...
			if len(rules) == 0 {
				newContentObject = nil
			} else {

				if rule, ok = rules[string(bidderRequest.BidderName)]; !ok {
					rule = defaultRule
				}
...
//  getTransparencyRules return content object transparency rule for a bidder and default rule
func getTransparencyRules(transparencyExt *openrtb_ext.TransparencyExt) (map[string]openrtb_ext.BidderContentTransparency, openrtb_ext.BidderContentTransparency) {

	rules := transparencyExt.Content

	// Dont send content object if no rule and default is not present
	defaultRule := openrtb_ext.BidderContentTransparency{
		Include: false,
	}
