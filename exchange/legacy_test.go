...
func TestSiteVideo(t *testing.T) {
	...
	_, errs := exchangeBidder.requestBid(context.Background(), ortbRequest, openrtb_ext.BidderRubicon, openrtb_ext.BidderRubicon, 1.0, currencyConverter.Rates(), &adapters.ExtraRequestInfo{}, true)
	if len(errs) > 0 {
...
func TestBidTransforms(t *testing.T) {
	...
	exchangeBidder := adaptLegacyAdapter(&mockAdapter)
	currencyConverter := currency.NewRateConverter(&http.Client{}, "", time.Duration(0))
	seatBid, errs := exchangeBidder.requestBid(context.Background(), newAppOrtbRequest(), openrtb_ext.BidderRubicon, openrtb_ext.BidderRubicon, bidAdjustment, currencyConverter.Rates(), &adapters.ExtraRequestInfo{}, true)
	if len(errs) != 1 {
...
func TestWithTargeting(t *testing.T) {
	...
	exchangeBidder := adaptLegacyAdapter(&mockAdapter)
	currencyConverter := currency.NewRateConverter(&http.Client{}, "", time.Duration(0))
	bid, errs := exchangeBidder.requestBid(context.Background(), ortbRequest, openrtb_ext.BidderAudienceNetwork, openrtb_ext.BidderAudienceNetwork, 1.0, currencyConverter.Rates(), &adapters.ExtraRequestInfo{}, true)
	if len(errs) != 0 {
