...
func TestBidderUniquenessGatekeeping(t *testing.T) {
	...
	var bidders []string
	for _, bidder := range CoreBidderNames() {
		if bidder != BidderSilverPush && bidder != BidderTripleliftNative && bidder != BidderAdkernelAdn && bidder != "freewheel-ssp" && bidder != "yahooAdvertising" && bidder != "boldwin_rapid" && !util.IsORTBBidder(bidder.String()) {
