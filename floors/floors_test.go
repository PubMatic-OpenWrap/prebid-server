...
type MockFetchDataRate struct{}

func (m *MockFetchDataRate) Fetch(configs config.AccountPriceFloors) (*openrtb_ext.PriceFloorRules, string) {
...
func TestResolveFloorsWithUseDataRate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			resolvedFloors, _ := resolveFloors(tc.account, tc.bidRequestWrapper, getCurrencyRates(rates), &MockFetchDataRate{})
			assert.Equal(t, resolvedFloors, tc.expFloors, tc.name)
		})
	}
}
